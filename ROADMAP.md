...
**Second corridor pass.** The 2026.2 sweep checked 89 of 372 corridor listings — 283 remain honestly unchecked (see [wiki/Data-statistics.md](wiki/Data-statistics.md) and [wiki/County-sweep-2026.2.md](wiki/County-sweep-2026.2.md)). The near-term goal is to finish the corridor and then take the high-traffic interior counties, prioritized by the feature requests that name them. This is the single most valuable work in the project and it is almost entirely a data problem, not a code problem.

**Watchlist re-checks.** The tier went live in 2026.2 with 44 entries, every one carrying its documented search in `reason`/`status_note`. The bar stays where it was — a documented zero-match or adverse-status search, terms and date recorded — see the [FAQ](wiki/FAQ.md). Next cycle: re-run those searches; a renewal moves an entry out.

**Expiry-awareness pass.** Licenses expiring 2027-08-31 (the seed record among them) age into the "expiring soon" window the apps already compute. The first real expiry event is a chance to prove the pipeline handles a status transition correctly (Clear → Expired, or renewed → new expiry date) and to document that story in the refresh log.
...
**CSV export — shipped in 2026.2.** The most-requested "give me the data" format landed as `web/data/licensed.csv` (+ root `data/licensed.csv` mirror), generated by the fan-out script from the same single source so it can never drift from the JSON. Remaining idea in this space: a per-county split of the CSV — an Issues-signal decision.
